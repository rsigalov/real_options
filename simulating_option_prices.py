...
# # Temporary function for the plain vanilla model with symmetric adjustment costs
# model_name = "options_1"
...
# model_params = metadata["params"]
# model_params["cfprop"] = 0.0
# model_params["phi"] = 5.0
# model_params["phim"] = 5.0
...
# vars_df_equity = pd.DataFrame(vars_list_equity, columns=["VarP", "VarQ"])
# vars_df_equity = sims_equity.join(vars_df_equity)
# vars_df_equity.to_csv(f"simulation_distributions/variances_equity_options_0.csv", index=False)
...
# start_time = timeit.default_timer()
# ivs_list_equity = []
# for inputs in tqdm(equity_inputs_list):
#     ivs_list_equity.append(
#         priceOptionsParallel(inputs)
#     )
# end_time = timeit.default_timer()
# print(f"processed in {end_time - start_time:.2f} s.")
...
# ivs_df_equity.to_csv(f"simulation_distributions/simulation_distributions_equity_options_0.csv", index=False)
...
# # Saving
# full_df_equity.to_csv(f"simulation_distributions/simulation_for_reg_equity_options_0.csv", index=False)
...
# P0, V1, H1, EM = debt_inputs_list[0]
...
# sigma_list = np.linspace(0.05, 0.09, num=100)
# plt.plot(sigma_list, [BS_put_price(P0, rf, K_grid[0], sigma, 1.0) for sigma in sigma_list])
# plt.axhline(P_grid[0])
# plt.axvline(IV_low)
...
# df_to_plot = sims_debt.pivot_table(index="ik", columns="ib", values="nobs", fill_value=np.nan, aggfunc=np.sum)
# mat_to_plot = np.array(df_to_plot)
# fig, ax = plt.subplots(figsize=(10, 4))
# ax.imshow(np.log(mat_to_plot).T)



# row = sims_debt.iloc[10]
# inputs = prepareParallelInputsEquity(model, row["ik"], row["ix"], row["iy"])
...
# # ix_path = np.ones(num_periods, dtype=int)*10

# # Fixing random variables before simulating (under the same seed)
# omega = 2.0
# num_prospective_entrants = 50
# np.random.seed(1)
# entrants_signals = np.random.pareto(omega, size=(num_periods, num_prospective_entrants))
...
# print("Simulating model without debt")
# model.simulate(ix_path, ik_private, centry, entrants_signals, entry_prod_u, prod_signals, debt=False)

# print("Simulating model with debt")
# model.simulate(ix_path, ik_private, centry, entrants_signals, entry_prod_u, prod_signals, debt=True)

# #%% If there is no exit in the model, can simulate a cross section of 1000 firms
# # by drawing from an unconditional distribution of (k, b, y)
...
# # Number of firms that exited
# model.sim_equity_df["exit"].sum()

# # Distribution of firm states at the last period
# sim_equity_last = model.sim_equity_df[model.sim_equity_df["t"] == model.sim_equity_df["t"].max()]

# # Drawing initial states of firms to simulate a fixed number of firms 
# # with stochastic aggregate state
# np.random.seed(2)
# initial_states = sim_equity_last[["ik", "iy"]].sample(3000)


# #%% For models with no exit: first, simulate for 



# #%% Plotting number of firms over time
# model.sim_debt_df.groupby("t")["id"].count().plot()

# #%% Calculating option prices, IVs and Skews for states that occur in the simulation

# sim_debt_subdf = model.sim_debt_df[
#     (model.sim_debt_df["t"] >= 526) & 
#     (model.sim_debt_df["t"] <= 926) & 
#     (model.sim_debt_df["exit"] == 0)
# ]

# distinct_states = sim_debt_subdf[["ik", "ib", "ix", "iy"]].drop_duplicates().reset_index()
# # row = model.sim_debt_df.iloc[400000]

# # priceOptions(model, row["ik"], row["ib"], row["ix"], row["iy"])

# # sample_states = distinct_states.sample(5000)
# print(f"Total iterations: {distinct_states.shape[0]}")
# entry_list = []
# for irow, row in tqdm(distinct_states.iterrows()):
#     opt = priceOptions(model, row["ik"], row["ib"], row["ix"], row["iy"])
#     atm_iv = opt[3][1]
#     skew = opt[3][0] - opt[3][1]
#     entry_list.append((atm_iv, skew))

# iv_df = pd.DataFrame(entry_list, columns=["iv", "skew"])
# assert iv_df.shape[0] == distinct_states.shape[0]
# iv_df = distinct_states.join(iv_df)
# sim_debt_subdf = pd.merge(sim_debt_subdf, iv_df, on=["ik", "ib", "ix", "iy"], how="left")

# # %% Preparing the data frame to estimate a regression similar to one
# # estimated in the data

# # 1. Calculating leverage, book-to-market
# sim_debt_subdf = sim_debt_subdf.sort_values(["id", "t"])
# sim_debt_subdf["bm"] = (sim_debt_subdf["k"] - sim_debt_subdf["b"])/sim_debt_subdf["V"]
# sim_debt_subdf["lev"] = sim_debt_subdf["b"]/sim_debt_subdf["k"]
# sim_debt_subdf["k_next"] = sim_debt_subdf.groupby("id")["k"].shift(-1)
# sim_debt_subdf["inv"] = sim_debt_subdf["k_next"] - sim_debt_subdf["k"]
# sim_debt_subdf["size"] = np.log(sim_debt_subdf["V"])
# sim_debt_subdf = sim_debt_subdf[sim_debt_subdf["inv"] >= 0]

# # 2. Calculating cross-sectional rankings for each variable
# for var in ["bm", "lev", "inv", "size"]:
#     sim_debt_subdf[f"{var}_csnorm"] = sim_debt_subdf.groupby("t")[var].rank(pct = True)*2 - 1

# # 3. Estimating regressions
# sim_debt_subdf.to_csv("toreg.csv", index=False)

# # Regression without interactions
# reg_iv = smf.ols(data = sim_debt_subdf, formula = "iv ~ bm + lev + inv + logME").fit()
# reg_iv.summary()
# reg_skew = smf.ols(data = sim_debt_subdf, formula = "skew ~ bm + lev + inv + logME").fit()

# # Regressions with interactions
# reg_iv_int = smf.ols(data = sim_debt_subdf, formula = "iv ~ bm + lev + inv + logME").fit()


# #%% Looking at behavior of a firm

# # Fixing a firm
# firm_id = 5000
# sim_firm = model.sim_debt_df[model.sim_debt_df["id"] == firm_id].reset_index().iloc[:20]
# sim_firm = sim_firm.sort_values("t")

# # Calculating option variables for this firm
# entry_list = []
# for irow, row in tqdm(sim_firm.iterrows()):
#     opt = priceOptions(model, row["ik"], row["ib"], row["ix"], row["iy"])
#     atm_iv = opt[3][1]
#     skew = opt[3][0] - opt[3][1]
#     entry_list.append((atm_iv, skew))
...
# iv_df = pd.DataFrame(entry_list, columns=["iv", "skew"])
# assert iv_df.shape[0] == sim_firm.shape[0]
# sim_firm = sim_firm.join(iv_df)

# # Creating additional variables
# sim_firm["bm"] = (sim_firm["k"] - sim_firm["b"])/sim_firm["V"]
# sim_firm["lev"] = sim_firm["b"]/sim_firm["k"]

# # Plotting
# var_list = ["ix", "iy", "k", "b", "V", "bm", "lev", "iv", "skew"]
# fig, axs = plt.subplots(len(var_list), 1, figsize=(5, 15), sharex=True)
# for var, ax in zip(var_list, axs):
#     sim_firm.set_index("t")[var].plot(ax=ax, marker="o")
#     ax.set_ylabel(var)
# fig.tight_layout()
